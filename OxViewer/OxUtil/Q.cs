...
namespace OxUtil
{
    public class Q
    {
        ...
        public Q(float x, float y, float z)
        {
            double angle;
        ...
        #region Operators
        //! equal operator
        public static bool operator ==(Q lhs, Q rhs)
        {
            if (lhs.X != rhs.X)
                return false;
            if (lhs.Y != rhs.Y)
        ...
        //! Sum operator
        public static Q operator +(Q lhs, Q rhs)
        {
            return new Q(lhs.X + rhs.X, lhs.Y + rhs.Y, lhs.Z + rhs.Z, lhs.W + rhs.W);
        }
        ...
        public static Q operator *(float lhs, Q rhs)
        {
            return new Q(lhs * rhs.X, lhs * rhs.Y, lhs * rhs.Z, lhs * rhs.W);
        }
        //! right multiplication by a float operator
        public static Q operator *(Q rhs, float lhs)
        {
            return new Q(lhs * rhs.X, lhs * rhs.Y, lhs * rhs.Z, lhs * rhs.W);
        }
        //! multiplication operator by a vector
        public static float[] operator *(float[] lhs, Q rhs)
        {
            float[] uv = new float[3];
            float[] uuv = new float[3];
            float[] qvec = new float[] { rhs.X, rhs.Y, rhs.Z };
            uv = MathHelper.Cross(qvec, lhs);
            uuv = MathHelper.Cross(qvec, uv);
            for (int i = 0; i < uv.Length; i++)
                uv[i] *= (2.0f * rhs.W);
            for (int i = 0; i < uuv.Length; i++)
                uuv[i] *= 2.0f;

            return new float[] { lhs[0] + uv[0] + uuv[0], lhs[1] + uv[1] + uuv[1], lhs[2] + uv[2] + uuv[2] };
        }
        // override Equals
        ...
        public static Q FromEulerAngles(float x, float y, float z)
        {
            Q t_tmp = new Q();
        ...
        public float GetDotProduct(Q q2)
        {
        ...
        public float[] Matrix
        {
            get
            {
                ...
                float[] frame_x = new float[]{
                          1.0f - 2.0f * m_y * m_y - 2.0f * m_z * m_z,
                          2.0f * m_x * m_y + 2.0f * m_z * m_w,
                          2.0f * m_x * m_z - 2.0f * m_y * m_w};
                frame_x = MathHelper.Normalize(frame_x);
                m[0] = frame_x[0];
                m[1] = frame_x[1];
                m[2] = frame_x[2];
                m[3] = 0.0f;

                float[] frame_y = new float[]{
                         2.0f * m_x * m_y - 2.0f * m_z * m_w,
                         1.0f - 2.0f * m_x * m_x - 2.0f * m_z * m_z,
                         2.0f * m_z * m_y + 2.0f * m_x * m_w};
                frame_y = MathHelper.Normalize(frame_y);
                m[4] = frame_y[0];
                m[5] = frame_y[1];
                m[6] = frame_y[2];
                m[7] = 0.0f;

                float[] frame_z = new float[]{
                         2.0f * m_x * m_z + 2.0f * m_y * m_w,
                         2.0f * m_z * m_y - 2.0f * m_x * m_w,
                         1.0f - 2.0f * m_x * m_x - 2.0f * m_y * m_y};
                frame_z = MathHelper.Normalize(frame_z);
                m[8] = frame_z[0];
                m[9] = frame_z[1];
                m[10] = frame_z[2];
                m[11] = 0.0f;
        ...
        public Q Slerp(Q q1, Q q2, float time)
        {
        ...
        public void fromAngleAxis(float angle, float[] axis)
        {
        ...
        /// <returns></returns>
        public float[] toEuler(out float[] euler)
        {
            euler = new float[3];
            ...
            euler[2] = (float)(Math.Atan2(2.0 * (m_x * m_y + m_z * m_w), (sqx - sqy - sqz + sqw)));

            // bank = rotation about x-axis
            euler[0] = (float)(Math.Atan2(2.0 * (m_y * m_z + m_x * m_w), (-sqx - sqy + sqz + sqw)));

            // attitude = rotation about y-axis
            euler[1] = (float)(Math.Asin(-2.0 * (m_x * m_z - m_y * m_w)));
            return euler;
        }
        #endregion
        #region Properties
