...
opvw.Plugin = opvw.Plugin || {
	...
	CLASS_ID : "clsid:6D0B8646-DA71-4a4b-AD47-FD6F2A145937",
	CODE_BASE : "http://3di-opensim.com/openviewer/product/3Di_OpenViewer.cab", // <-- I comment out place used. Please serch "CODE_BASE".
	 ...
	 */
	start : function( targetID, width, height, onEvent){
		var doc = opvw.context.document;
		if( typeof doc != 'undefined' && 
			targetID != undefined ){
			...
			if (opvw.isMSIE()) {	// for IE
				...
				{
					data += '<script language="javascript" type="text/jscript" for="' + opvw.Plugin.PLUGIN_ID + '" event="OnEvent(message)">';
					data += 'opvw.Plugin.onEvent(message);';
					data += '</script>';
				}
				...
				// Register Event
				embedTag.SetEventListener(opvw.Plugin.onEvent);
			}
	...
	callFunction: function(message) {
		return opvw.API.plugin.Function(message);
	},
